...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/trajectories_level1_level2}"
mkdir -p "${OUTPUT_DIR}"
...
_run() {
  ...
  echo "=== Level 1+2 started $(date -Iseconds) ==="
  ...
  if [[ -n "${MAPPED_MODEL_NAME}" ]]; then
    echo "model-name: ${MAPPED_MODEL_NAME}"
  fi
  ...
  PYTHONPATH="src:.:${PYTHONPATH:-}" python3 -m minisweagent.run.benchmarks.swebench_mapped \
    ...
    "${model_args[@]}" \
    --output "${OUTPUT_DIR}"
  echo "=== Level 1+2 finished $(date -Iseconds) ==="
}

export FILTER_FILE TRANSLATED_FILE LEVEL2_MAPPING_DIR LEVEL2_SEMANTIC_SEED MAPPED_MODEL_CONFIG MAPPED_MODEL_NAME MAPPED_LITELLM_REGISTRY_PATH OUTPUT_DIR PROJECT_DIR TMPDIR
